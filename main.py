...
TOKEN = os.getenv("TELEGRAM_TOKEN", "8611511789:AAFzO_6nAGenCW2Hb4gI14J8LtZZHhojst4")
    ...
    # --- MODO MULTIMEDIA ---
        ...
        try:
            ...
            if duracion > MAX_DURATION:
                minutos_video = duracion // 60
                segundos_video = duracion % 60
                await status_msg.edit_text(
                    f"⚠️ **Video demasiado largo**\n\nEl límite permitido es de **5:00 minutos**.\nEl enlace enviado dura **{int(minutos_video)}:{int(segundos_video):02d}**.\n\nPor favor, intenta con un video más corto.",
                    parse_mode="Markdown"
                )
            ...
            # Hook dinámico asíncrono
            def progreso_hook(d):
                nonlocal ultima_actualizacion
                if d['status'] == 'downloading':
                    ahora = time.time()
                    if ahora - ultima_actualizacion > 1.5:
                        total = d.get('total_bytes') or d.get('total_bytes_estimate', 0)
                        descargado = d.get('downloaded_bytes', 0)
                        
                        if total > 0:
                            porcentaje = int((descargado / total) * 100)
                            barra_grafica = construir_barra(porcentaje)
                        else:
                            barra_grafica = "[░░░░░░░░░░] Descargando..."

                        texto_progreso = f"📥 **Descargando:** `{titulo[:30]}...`\n\n`{barra_grafica}`"
                        asyncio.run_coroutine_threadsafe(status_msg.edit_text(texto_progreso, parse_mode="Markdown"), loop)
            ...
            if es_audio:
                ydl_opts.update({'format': 'bestaudio/best', 'outtmpl': 'downloads/%(title)s.mp3'})
            else:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                path_final = ydl.prepare_filename(info_descarga)

            await status_msg.edit_text("⚡ **Enviando archivo a Telegram...**\n`[██████████] 100% Complete`", parse_mode="Markdown")
            
            if es_audio:
                await update.message.reply_audio(audio=open(path_final, 'rb'), caption=f"🎵 **{titulo}**\n\n_MP3 Extraído con éxito._", parse_mode="Markdown")
            else:
                await update.message.reply_video(video=open(path_final, 'rb'), caption=f"📹 **{titulo}**\n\n_Video listo._", parse_mode="Markdown")
            ...
            await status_msg.edit_text("❌ **Error:** El enlace es privado, incompatible o falló el procesamiento.", parse_mode="Markdown")
        
        context.user_data['modo_audio'] = False
    else:
        await update.message.reply_text("👋 Selecciona una opción del menú para empezar.", reply_markup=menu_principal())

# 5. MANEJADOR DE BOTONES (CALLBACKS)
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if "Message is not modified" not in str(e): logger.error(f"Error en botones: {e}")
...
async def startup_event():
    ...
    if WEBHOOK_URL:
        ...
        logger.warning("No se detectó RENDER_EXTERNAL_URL. Configura las variables de entorno.")
